	...
	private async _invoke(params: InvokeChangesetOperationParams, token: CancellationToken, signal: AbortSignal): Promise<InvokeChangesetOperationResult> {
		...
		if (!authToken) {
			throw new ProtocolError(
				AHP_AUTH_REQUIRED,
				localize('agentHost.changeset.commit.authRequired', "Configure Frame AI to generate a commit message."),
				[copilotResource],
			);
		}
		...
		try {
			...
			if (this._isAuthFailure(err)) {
				throw new ProtocolError(
					AHP_AUTH_REQUIRED,
					localize('agentHost.changeset.commit.authExpired', "Authentication or local model setup is required to generate a commit message. Check Frame AI and try again."),
					[copilotResource],
				);
			}
			throw err;
