...
export const agentHostCustomizationConfigSchema = createSchema({
	...
	[AgentHostConfigKey.ClaudeUseCopilotProxy]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.claudeUseCopilotProxy.description', "When enabled (the default), the Claude agent routes all requests through Frame AI. When disabled, Claude talks to Anthropic directly using your own credentials (API key or Claude subscription)."),
		default: true,
	}),
