...
/**
 * Root-config keys consumed exclusively by the local agent CLI provider
...
export const enum CopilotCliConfigKey {
	...
	/** Log level passed to the agent host SDK client. */
...
export const copilotCliConfigSchema = createSchema({
	[CopilotCliConfigKey.EnableCustomTerminalTool]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.enableCustomTerminalTool.description', "When enabled, agent host SDK sessions use Agent Host's terminal tool override instead of the SDK's default terminal behavior."),
		default: false,
	}),
	[CopilotCliConfigKey.CopilotSdkLogLevel]: schemaProperty<CopilotSdkLogLevelSetting>({
		type: 'string',
		title: localize('agentHost.config.copilotSdkLogLevel.title', "agent host SDK Log Level"),
		description: localize('agentHost.config.copilotSdkLogLevel.description', "Controls logging from the agent host SDK runtime. Agent host trace logging always enables trace output."),
	...
	[CopilotCliConfigKey.Opus48Prompt]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.opus48Prompt.description', "When enabled, agent host SDK sessions running a Claude Opus 4.8 model apply Opus 4.8-tuned system-prompt section overrides on top of the default system message."),
		default: false,
	}),
	[CopilotCliConfigKey.ReasoningEffortOverride]: schemaProperty<string>({
		...
		description: localize('agentHost.config.reasoningEffortOverride.description', "Overrides the reasoning effort for agent host SDK sessions regardless of the per-model picker value. Set it to a level the selected model supports (e.g. `low`, `medium`, `high`, `xhigh`); a value that isn't a recognized effort level is ignored and the session falls back to the picker value. Only affects agent host SDK sessions; intended for experimentation."),
		default: '',
	}),
	[CopilotCliConfigKey.ModelCapabilityOverrides]: schemaProperty<CopilotCliModelCapabilityOverrides>({
		...
		description: localize('agentHost.config.modelCapabilityOverrides.description', "Per-model capability overrides for agent host SDK sessions, keyed by model id. Aliasing a model id to a known `family` routes it to that family's tuned system prompt without changing the model id sent to the runtime. Only affects agent host SDK sessions; intended for experimentation."),
