...
/** Configuration key controlling automatic OS system proxy discovery for agent-host chat sessions. */
 ...
 * workbench surfaces the agent host's Claude provider (vs. the Frame AI
...
/**
 * Configuration key that controls the sandbox mode for the agent host SDK's built-in
 ...
 *  - `false` (default) — Claude is provided by the Frame AI extension.
 *
...
/**
 * Canonical {@link ProtectedResourceMetadata} for the Frame AI
 * resource. Shared between every agent provider that consumes a GitHub
 * Copilot bearer token (e.g. local agent CLI, Claude) so they advertise an
...
/**
 * Implemented by each agent backend (e.g. agent host SDK).
...
export interface IAgentService {
	...
	/** List all available sessions from the local agent CLI. */
	listSessions(): Promise<IAgentSessionMetadata[]>;
...
export interface IAgentConnection {
	 ...
	 * remote), so the result reflects the environment the agent host SDK actually
	 * runs in.
	 */
	getNetworkDiagnosticsInfo(): Promise<IAgentHostNetworkDiagnosticsInfo>;
	 ...
	 * environment the agent host SDK actually runs in.
	 */
	diagnosticsFetch(url: string): Promise<IAgentHostNetworkFetchResult>;
