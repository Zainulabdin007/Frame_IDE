...
configurationRegistry.registerConfiguration({
	...
	properties: {
		[AgentHostSystemProxyEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.systemProxy.enabled', "When enabled, chat sessions automatically discover and use the operating system's proxy configuration when no proxy environment variable is set."),
			default: true,
			...
			policy: {
				...
				localization: {
					description: {
						...
						value: nls.localize('chat.agentHost.claudeAgent.enabled.policy', "Enable Claude Agent sessions. Not used by Frame local AI (no cloud subscriptions)."),
					}
				}
			},
		},
			...
			policy: {
				...
				localization: {
					description: {
						...
						value: nls.localize('chat.agentHost.codexAgent.enabled.policy', "Enable Codex Agent sessions. Not used by Frame local AI (no cloud subscriptions)."),
					}
				}
			},
		},
		...
		[AgentHostOTelExporterTypeSettingId]: {
			...
			markdownDescription: nls.localize('chat.agentHost.otel.exporterType', "Exporter backend used by the agent host SDK when `#chat.agentHost.otel.enabled#` is on. Configurable in user settings only. `otlp-grpc` is downgraded to `otlp-http` transparently in the CLI runtime."),
			default: 'otlp-http',
			scope: ConfigurationScope.APPLICATION,
			tags: ['experimental', 'advanced'],
		...
		['chat.agentHost.otel.headers']: {
			...
			included: false,
			tags: ['experimental', 'advanced'],
			markdownDescription: nls.localize('chat.agentHost.otel.headers', "Enterprise-managed OTLP exporter headers (e.g. auth tokens) for Copilot OpenTelemetry export. Policy-only and extension-only: applied directly to Frame AI Chat extension's OTLP exporter, never delivered to the agent host process."),
			policy: {
