...
export class ExtensionStatusAction extends ExtensionAction {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.enablementState === EnablementState.DisabledByUnification) {
			this.updateStatus({ icon: infoIcon, message: new MarkdownString(localize('extension disabled because of unification', "Built-in AI features are managed by Frame. Toggle {0} if you need to change extension unification behavior.", '`chat.extensionUnification.enabled`')) }, true);
			return;
		}
