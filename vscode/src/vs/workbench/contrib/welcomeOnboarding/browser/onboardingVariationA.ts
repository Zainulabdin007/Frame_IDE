 ...
 *
 * Steps:
 * 1. Sign In — sessions-style sign-in hero with Frame AI, Google, and Apple options
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to Frame"));
	...
	private _updateButtonStates(): void {
		...
		if (this.footerLeft) {
			// Frame: no "Open Frame AI" footer nudge
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to Frame");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Frame AI runs locally — no account required.");
		...
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.copilotPrefix', ". Frame AI runs locally and does not send your code to cloud AI providers. "));
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.improveSuffix', "You can change related "));
		const settingsLink = this._createInlineLink(copilotDisclaimer, localize('onboarding.signIn.disclaimer.settings', "settings"), '#');
		settingsLink.removeAttribute('target');
		settingsLink.removeAttribute('rel');
		this.stepDisposables.add(addDisposableListener(settingsLink, EventType.CLICK, e => {
			e.preventDefault();
			void this.commandService.executeCommand('workbench.view.frameAI');
		}));
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all Frame commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // Frame default, nothing to install
		}
	...
	private _renderAgentSessionsStep(container: HTMLElement): void {
		...
		this._createFeatureCard(moreGrid, Codicon.rocket,
			...
			localize('onboarding.sessions.runAnywhere.desc', "Run agents locally for interactive work with Frame AI. Cloud and background cloud agents are not part of Frame."));

		this._createFeatureCard(moreGrid, Codicon.settingsGear,
			localize('onboarding.sessions.customize', "Customize Your Agents"),
			localize('onboarding.sessions.customize.desc', "Tailor Frame AI to your project with custom instructions and agents, skills, reusable prompts, and MCP servers that connect to the tools and context you rely on."));
