 ...
 * the active chat is a first-party agent-host session (local agent CLI today)
 ...
 * widget is showing a contributed session (local agent CLI, Cloud, Claude, etc.)
...
export class AgentsHandoffInputTipContribution extends Disposable implements IWorkbenchContribution {
	...
	private _update(): void {
		...
		// Empty-workspace + local local agent CLI: the local agent host can't
		...
		const message = useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.message', "Local agent host isn't available without an open folder")
		...
		const description = useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.description', "Open the Agents Window to start a local agent session.")
			: mode === AgentsHandoffTipMode.Custom
				? localize('chat.agentsHandoff.tip.description.copilot', "Open a dedicated multi-pane Agents view alongside your workspace.")
