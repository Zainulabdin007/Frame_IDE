...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// Frame: mark residual Copilot chat setup complete so SetupAgent never prompts for sign-in.
		this.logService.info('[Frame] Marking chat setup completed (local Frame AI, no Microsoft account)');
		void context.update({ completed: true });
		void context.update({ later: false });
		void context.update({ hidden: false });
		...
		const updateRegistration = () => {
			...
			// Agent + Tools
			{
				if (!context.state.hidden && !context.state.disabledInWorkspace) {
					...
					// Frame: default Chat / inline agents are owned by FrameChatContribution
					// (frame.chat / frame.edits / frame.agent). Do not register SetupAgent as
					// isDefault — that blocked Frame Intelligence behind a redirect.
					if (!defaultAgentDisposables.value) {
						defaultAgentDisposables.value = new DisposableStore();
						this.logService.info('[Frame] Skipping SetupAgent default registration — Frame Chat owns defaults');
					}
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		class ChatSetupTriggerAction extends Action2 {
			...
			static CHAT_SETUP_ACTION_LABEL = localize2('triggerChatSetup', "Open Frame AI Chat");
			...
			constructor() {
				super({
					...
					f1: false, // Frame: hide Copilot setup from Command Palette
		...
		class ChatSetupTriggerForceSignInDialogAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('forceSignIn', "Open Frame AI Chat"),
					f1: false,
					precondition: ContextKeyExpr.false(), // Frame: no Copilot force sign-in
				});
			}
		...
		class ChatSetupFromAccountsAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('triggerChatSetupFromAccounts', "Open Frame AI..."),
					menu: {
						id: MenuId.AccountsContext,
						group: '2_copilot',
						when: ContextKeyExpr.false(), // Frame: no Accounts "Sign in to Copilot"
					}
				});
			}
			...
			constructor() {
				super({
					...
					menu: [{
						...
						when: ContextKeyExpr.false(), // Frame: no title-bar Sign In button
					}]
				});
			}
		...
		class ToggleSignInTitleBarAction extends ToggleTitleBarConfigAction {
			constructor() {
				super(
					...
					localize('toggle.chatSignIn', 'Frame AI Sign In'),
					localize('toggle.chatSignInDescription', "Toggle visibility of the Frame AI Sign In button in title bar"),
					3,
					ContextKeyExpr.and(
						IsWebContext.negate(),
		...
		class UpgradePlanAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('managePlan', "Frame AI (local)"),
					category: localize2('chat.category', 'Chat'),
					f1: false, // Frame: no Frame AI upgrade in Command Palette
					precondition: ContextKeyExpr.false(),
					menu: {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				// Frame: no GitHub upgrade — open local Frame sidebar instead.
				await commandService.executeCommand('workbench.view.frameAI');
			}
		}
		...
		class ManageAdditionalSpendAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('manageAdditionalSpend', "Frame AI Budget"),
					category: localize2('chat.category', 'Chat'),
					f1: false, // Frame: no Copilot budget management
					precondition: ContextKeyExpr.false(),
					menu: {
						id: MenuId.ChatTitleBarMenu,
						group: 'a_first',
...
class ChatSetupExtensionUrlHandler implements IExtensionUrlHandlerOverride {
	...
	private async _handleUpgradeSuccess(): Promise<boolean> {
		...
		this.chatInputNotificationService.setNotification({
			...
			message: localize('upgradeSuccess', "Frame AI Ready"),
			description: localize('upgradeSuccessDescription', "Your local Frame AI configuration is ready."),
			actions: [],
			dismissible: true,
