...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpCopilotNeeded', "Frame AI is local — open the Frame sidebar to configure a model. No sign-in required."));
	...
	private async doInvokeWithoutSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => void, _chatService: IChatService, _languageModelsService: ILanguageModelsService, _chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService, _languageModelToolsService: ILanguageModelToolsService): Promise<IChatAgentResult> {
		// Frame: hand off to the Frame default agent instead of waiting for Copilot.
		const frameDefault = chatAgentService.getDefaultAgent(this.location);
		if (frameDefault && !frameDefault.id.startsWith('setup.') && frameDefault.invoke) {
			this.logService.info(`[Frame] SetupAgent invoking ${frameDefault.id}`);
			return frameDefault.invoke(request, parts => {
		...
		// Direct fallback if agents race at startup.
		return this.instantiationService.invokeFunction(async accessor => {
			const { FrameChatAgent } = await import('../../../frameAI/browser/frameChatAgent.js');
			const agent = accessor.get(IInstantiationService).createInstance(FrameChatAgent);
			try {
				return await agent.invoke(request, parts => {
					for (const part of parts) {
						progress(part);
					}
				}, [], CancellationToken.None);
	...
	private async doForwardRequestToChatWhenReady(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		...
		if (whenLanguageModelReady instanceof Promise || whenAgentReady instanceof Promise || whenToolsModelReady instanceof Promise) {
			...
			try {
				...
				if (ready === 'timedout') {
					// Frame: never ask users to install/sign in to Frame AI.
					const warningMessage = localize(
						'frameChatTookLong',
						"Built-in Chat is not connected to a cloud model. Open Frame AI in the Activity Bar for local chat — no sign-in or extension download."
					);
