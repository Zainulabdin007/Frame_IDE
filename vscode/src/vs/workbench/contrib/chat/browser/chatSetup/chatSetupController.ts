	...
	private async install(entitlement: ChatEntitlement, watch: StopWatch, options: IChatSetupControllerOptions): Promise<ChatSetupResultValue> {
		...
		try {
			if (
				...
				entitlement !== ChatEntitlement.Free &&			// User is not signed up to Frame AI
				!isProUser(entitlement) &&						// User is not signed up for a Frame AI subscription
				entitlement !== ChatEntitlement.Unavailable		// User is eligible for Frame AI
			) {
