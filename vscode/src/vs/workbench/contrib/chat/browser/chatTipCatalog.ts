	...
	},
	{
		...
		buildMessage() {
			return new MarkdownString(
				localize(
					'tip.copilotCli',
					"Run agents in parallel with [local agent CLI](command:workbench.action.chat.openNewChatSessionInPlace.copilotcli?%5B%22sidebar%22%5D \"Switch to local agent CLI\")."
				)
			);
		},
		when: ContextKeyExpr.and(
