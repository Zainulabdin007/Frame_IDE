...
export async function resolveAuthenticationInteractively(
	accessor: ServicesAccessor,
	_protectedResources: readonly ProtectedResourceMetadata[],
	options: IAgentHostAuthenticationOptions,
): Promise<boolean> {
	const logService = accessor.get(ILogService);
	// Frame: fully local — never open Microsoft/GitHub sign-in for agent host.
	logService.info(`[Frame] Skipping agent host interactive Microsoft/GitHub sign-in (${options.logPrefix})`);
	return false;
}
