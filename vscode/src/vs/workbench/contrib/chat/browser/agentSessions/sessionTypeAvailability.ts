...
export enum SessionTypeAvailability {
	...
	Available,
	/** Unusable until the user signs in (the type needs a Frame AI account; BYOK is not supported here). */
	SignInRequired,
	/** Unusable, but the user can resolve it by upgrading (Frame AI / Student). */
	UpgradeRequired,
 ...
 * targeted model. When a type is not usable, Frame AI / Student (EDU) users
...
export function getSessionTypeAvailability(
	...
	// Copilot-backed types need a Frame AI account (BYOK isn't supported here), so a signed-out user can't use them —
	...
	// (e.g. the local agent CLI harness) makes it usable.
...
export function getSessionTypeUnavailableDescription(availability: SessionTypeAvailability): IMarkdownString | undefined {
	switch (availability) {
		...
		case SessionTypeAvailability.UpgradeRequired:
			// Frame: cloud agents are not part of the product — no Copilot sign-in/upgrade CTAs.
			return new MarkdownString(localize('chat.sessionType.frameUnavailable', "Not available in Frame (local AI only)"));
...
export function getSessionTypeUnavailableHover(availability: SessionTypeAvailability): IMarkdownString | undefined {
	switch (availability) {
		case SessionTypeAvailability.SignInRequired:
		case SessionTypeAvailability.UpgradeRequired:
			return new MarkdownString(localize('chat.sessionType.frameUnavailableHover', "Frame uses local models only. Configure a GGUF in the Frame sidebar."));
		case SessionTypeAvailability.NoModels:
...
export function getSessionTypeUnavailableLabel(availability: SessionTypeAvailability): string | undefined {
	switch (availability) {
		case SessionTypeAvailability.SignInRequired:
		case SessionTypeAvailability.UpgradeRequired:
			return localize('chat.sessionType.frameUnavailableMobile', "Not available in Frame (local AI only)");
		case SessionTypeAvailability.NoModels:
