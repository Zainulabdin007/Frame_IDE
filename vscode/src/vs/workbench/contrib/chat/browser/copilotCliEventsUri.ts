 ...
 *
 * Used for both the local Agent Host local agent CLI provider and the
 * extension-host local agent CLI provider, which share the same on-disk layout
...
/**
 * Extracts the raw local agent CLI session id from a chat session resource.
 */
 ...
 * `events.jsonl` URI for the corresponding local agent CLI session, or
...
/**
 * Resolves the host-local filesystem path string of a local agent CLI session's
 ...
 * used by the chat debug panel and the "Open local agent CLI State File" command,
 ...
 *
 * Returns `undefined` when the session is not a local agent CLI session or the
