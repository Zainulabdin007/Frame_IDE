...
export class VoiceSessionController extends Disposable implements IVoiceSessionController {
				...
				const autorunDisposable = autorun(reader => {
					...
					for (const s of agentSessions) {
						...
						if (model) {
							...
							// Remote/chat sessions don't keep their model resident, so a
	...
	private _checkSessionStateChanges(): void {
		...
		for (const s of sessions) {
			...
			// Summary-less idle transitions for remote/chat sessions: narrate
	 ...
	 * chat sessions don't keep their model loaded, so without this the
