...
export class ChatQuotaNotificationContribution extends Disposable implements IWorkbenchContribution {
	...
	private _update(): void {
		...
		// time quota runs out. Done before Frame AI/BYOK gate so a recovery is
	...
	private _showQuotaTrajectoryWarning(warning: { averageDailyUsage: number; percentUsed: number }): void {
		...
		const message = localize({ key: 'quota.trajectory.message', comment: ['{Locked="["}', '{Locked="]({0})"}'] }, "Local AI usage is high. Check your Frame sidebar runtime if generation slows. {0}.", learnMoreLink);
	...
	private _showExhaustedNotification(): void {
		...
		if (entitlement === ChatEntitlement.Unknown) {
			...
			actions = [{ kind: ChatInputNotificationActionKind.Command, label: localize('manageBudget', "Frame AI Settings"), commandId: 'workbench.action.chat.manageAdditionalSpend' }];
		} else {
			description = localize('quota.exhausted.default', "Manage your budget to keep building.");
			actions = [{ kind: ChatInputNotificationActionKind.Command, label: localize('manageBudget2', "Frame AI Settings"), commandId: 'workbench.action.chat.manageAdditionalSpend' }];
		}
	...
	private _showQuotaApproachingWarning(warning: { percentUsed: number; threshold: number }): void {
		...
		if (entitlement === ChatEntitlement.Unknown || entitlement === ChatEntitlement.Free) {
			...
			if (this._switchToAutoTreatment === true && canSwitchToAuto) {
				...
				actions = [{ kind: ChatInputNotificationActionKind.Command, label: localize('manageBudget3', "Frame AI Settings"), commandId: 'workbench.action.chat.manageAdditionalSpend' }];
			}
		}
	...
	/**
	 * Frame: never treat the local chat path as Copilot for quota chrome.
	 */
	...
	private _showManagedPlanBlockedNotification(): void {
		...
		this._setNotification({
			...
			description: localize('quota.blocked.managed', "Local AI usage is paused. Contact your administrator if this persists."),
			actions: [],
			dismissible: true,
			autoDismissOnMessage: true,
		});
