...
export interface IChatStatusDashboardOptions {
	...
	/**
	 * When provided, CTA buttons (Frame AI Settings, Upgrade) are rendered into
...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		// Frame: never show Copilot quota / upgrade / manage-settings chrome.
		const hasUsageSection = false;
		const hasVisibleUsageContent = false;
		void chat; void completions;
		...
		if (hasUsageSection && !this.options?.compactQuotaLayout) {
			...
			const header = this.renderHeader(headerHost, this._store, planName, toAction({
				...
				label: localize('quotaLabel', "Frame AI Settings"),
				tooltip: localize('quotaTooltip', "Frame AI Settings"),
				class: ThemeIcon.asClassName(Codicon.settings),
				run: () => this.runCommandAndClose('workbench.view.frameAI'),
			}));
			...
			if (canConfigureAdditionalSpend) {
				...
				headerAdditionalSpendButton.label = localize('manageBudget', "Frame AI Settings");
				this._store.add(headerAdditionalSpendButton.onDidClick(() => {
					...
					this.runCommandAndClose('workbench.view.frameAI');
				}));
				if (actionBarElement) {
		...
		// Premium chat included indicator — Frame: never show Copilot premium quota chrome.
		const hasPremiumUnlimited = false;
		void premiumChat;
		...
		if (chatQuota || premiumChatQuota || completionsQuota) {
			...
			if (premiumChatQuota && !premiumChatQuota.unlimited && premiumChatQuota.percentRemaining >= 0) {
				...
				const premiumChatLabel = isUBB
					? localize('creditsLabel', "Credits")
					: this.chatEntitlementService.quotas.additionalUsageEnabled ? localize('includedPremiumChatsLabel', "Included requests") : localize('premiumChatsLabel', "Requests");
	...
	private renderSetupSection(): void {
		// Frame: local-first — never show Copilot sign-in / setup / upgrade CTAs in the status dashboard.
	}
	...
	private createGlobalQuotaCallout(container: HTMLElement): () => { calloutVisible: boolean; additionalUsageEnabled: boolean } {
		...
		const update = () => {
			...
			if (isEnterpriseUser && isPooledQuotaExhausted) {
				...
				calloutText.textContent = localize('quotaBudgetExceededEnterprise', "Local AI usage is paused. Contact your administrator if this persists.");
			} else if (maxUsedPercentage >= 100 && additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageActiveEnterprise', "Local AI is paused. Contact your administrator if this persists.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageApproachingEnterprise', "Local AI may pause when limits are reached. Contact your administrator if needed.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaPausedEnterprise', "Local AI is paused until the limit resets. Contact your administrator if needed.")
					: localize('quotaPaused', "Local AI is paused until the limit resets.");
			} else if (maxUsedPercentage >= 75 && !additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaWarningEnterprise', "Local AI may pause when the limit is reached. Contact your administrator if needed.")
					: localize('quotaWarning', "Local AI may pause when the limit is reached.");
			} else {
				quotaCallout.style.display = 'none';
			}
