...
/**
 * Shared implementation of "Open local agent CLI State File". Resolves the
 ...
 * this helper after resolving the active local agent CLI session resource.
 */
export async function openCopilotCliStateFile(
	...
	switch (result.kind) {
		case 'ok':
			...
			return;
		case 'no-session':
			notificationService.info(localize('openSessionEventsFile.noSession', "No local agent CLI session is active."));
			return;
		case 'unsupported-scheme':
			notificationService.info(localize('openSessionEventsFile.unsupported', "The active chat session is not a local agent CLI session."));
			return;
 ...
 * find the active local agent CLI chat session. Suitable for vscode where the
...
export class OpenCopilotCliStateFileAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openSessionEventsFile', "Open local agent CLI State File"),
			f1: true,
