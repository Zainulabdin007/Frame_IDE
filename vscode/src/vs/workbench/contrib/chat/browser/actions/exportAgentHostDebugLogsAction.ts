 ...
 * local agent CLI session events file (if available), the window/shared/local
...
export async function collectAgentHostDebugLogs(
	...
	// 5. agent host SDK process logs under ~/.copilot/logs do not include the
 ...
 * or `undefined` if the scheme does not belong to a local agent CLI agent-host
