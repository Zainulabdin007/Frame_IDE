...
/** Bound the best-effort scan of agent host SDK process logs. */
...
export const enum AgentHostLogSourceKind {
	/** The local agent CLI `events.jsonl` model/conversation stream. */
	Events = 'events',
	/** The client-side AHP JSON-RPC wire log (`<logsHome>/ahp/*.jsonl`). */
	WireLog = 'wire',
	/** The agent host SDK process logs under `~/.copilot/logs`. */
	CliLog = 'cliLog',
 ...
 * remote local agent CLI). Only these sessions have AHP logs and agent-host
...
export async function enumerateAgentHostLogSources(
	...
	// 5. agent host SDK process logs (~/.copilot/logs), content-filtered lazily by session id.
	...
	if (rawSessionId) {
		...
		if (copilotLogsDir) {
			sources.push({
				id: 'cliLog',
				label: localize('agentHostLogs.cliLog', "local agent CLI Logs"),
...
/**
 * Scans a Frame AI logs directory for `.log` files whose content mentions the
