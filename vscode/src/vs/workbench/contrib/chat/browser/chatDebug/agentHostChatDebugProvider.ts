...
/**
 * One record in an Agent Host local agent CLI `events.jsonl` stream. The CLI
...
/**
 * Feeds Agent Host (local agent CLI) sessions into the Agent Debug Logs panel by
...
export class AgentHostChatDebugContribution extends Disposable implements IWorkbenchContribution {
	...
	private _ensureLiveRefresh(sessionResource: URI, eventsUri: URI): void {
		...
		// the single `events.jsonl`: the external local agent CLI process writes
	...
	private async _provideChatDebugLog(sessionResource: URI, token: CancellationToken): Promise<IChatDebugEvent[] | undefined> {
		...
		if (!eventsUri) {
			return undefined; // not an Agent Host local agent CLI session
		}
	 ...
	 *
	 * The local agent CLI appends to `events.jsonl` line-by-line from a separate
	...
	private async _discoverLocalSessions(token: CancellationToken): Promise<{ uri: URI; title?: string }[]> {
		...
		try {
			...
			return []; // no local local agent CLI sessions on disk
		}
		if (token.isCancellationRequested) {
			return [];
		}
 ...
 *
 * The record `parentId` is **not** a logical parent: the agent host SDK documents
...
function fallbackSessionTitle(sessionId: string): string {
	return localize('agentHost.debug.untitledSession', "local agent CLI Session {0}", sessionId.slice(0, 8));
}
