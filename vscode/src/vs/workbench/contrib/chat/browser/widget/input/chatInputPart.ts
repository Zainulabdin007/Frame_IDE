...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	*/
			...
			if (!state && this._chatSessionIsEmpty) {
				...
				if (state && this.getConfiguredModelValue()) {
					...
					if (configuredModel) {
						...
						// (e.g. Frame AI extension is still fetching its model list, so
	 ...
	 * Frame AI / Student user. In this state there is no model to send a
	...
	private ensureNotificationWidget(): void {
		if (!this._notificationWidget.value) {
			...
			// (e.g. empty workspace + local agent CLI [Agent Host] selected). Without
