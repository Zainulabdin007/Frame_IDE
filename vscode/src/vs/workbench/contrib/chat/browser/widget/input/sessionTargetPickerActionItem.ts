...
import { ChatConfiguration, isVisibleEditorChatSessionType, recordUserSelectedSessionType } from '../../../common/constants.js';
...
export class SessionTypePickerActionItem extends ChatInputPickerActionViewItem {
	...
	private _updateAgentSessionItems(): void {
		...
		// Frame: only expose the local session type — no Claude / Codex / local agent CLI / cloud agents.
		const agentSessionItems = allAgentSessionItems;
	...
	protected _getDefaultSessionType(): AgentSessionTarget {
		// Frame: always start on local sessions — never Agent Host / cloud defaults.
		return AgentSessionProviders.Local;
	}
