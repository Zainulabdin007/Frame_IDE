...
/**
 * Id of the synthetic "Use Frame AI..." entry shown when Chat still
...
function getVendorDisplayName(languageModelsService: ILanguageModelsService, vendor: string): string {
	if (vendor === 'copilotcli') {
		// @vritant24: This is temporary until we we have 2 distinct vendors for local agent CLI vs Copilot Chat.
		// For now, we want to show "Frame AI" in the model picker for both.
		return localize('chat.modelPicker.copilotGroup', "Local");
	}
 ...
 * "Use Frame AI" header and a Sign In action (invoking
...
export function buildModelPickerItems(
	...
	if (setupRequired) {
		// Frame: no Sign In prompt — local-first IDE does not require an account
		items.push({
			kind: ActionListItemKind.Header,
			label: localize('chat.modelPicker.setupRequired.frame', "Use local models via Frame AI"),
		});
		return items;
	}
	if (models.length === 0) {
		if (!showAutoModel) {
			...
			// disabled "No models available" entry. For Frame AI / Student
			...
			hover.appendMarkdown(localize('chat.modelPicker.upgradeHover.frame', "Configure a local GGUF model in the Frame sidebar."));
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		description = localize('chat.modelPicker.configureLocal.frame', "Configure local model");
	...
	if (reason === 'upgrade') {
		...
		if (chatEntitlementService.entitlement === ChatEntitlement.Pro) {
			hoverContent.appendMarkdown(localize('chat.modelPicker.upgradeHoverProPlus.frame', "Configure a local GGUF model in the Frame sidebar."));
		} else {
			hoverContent.appendMarkdown(localize('chat.modelPicker.upgradeHover.frame', "Configure a local GGUF model in the Frame sidebar."));
		}
	} else if (reason === 'update') {
