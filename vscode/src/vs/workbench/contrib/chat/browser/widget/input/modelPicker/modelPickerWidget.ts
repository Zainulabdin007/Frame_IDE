...
export class ModelPickerWidget extends Disposable {
	...
	private _requiresSetup(): boolean {
		// Frame: local-first — never block the model picker on cloud Chat setup / sign-in.
		return false;
	}
	...
	show(anchor?: HTMLElement): void {
		...
		const manageSettingsUrl = 'command:workbench.view.frameAI';
	...
	private _renderLabel(): void {
		...
		const ariaLabel = restrictedMode
			...
			: setupRequired
				? localize('chat.modelPicker.ariaLabelSetupRequired', "Models, configure Frame local AI")
