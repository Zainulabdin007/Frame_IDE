...
export interface IModelPickerDelegate {
	 ...
	 * "No models available" entry (and an upgrade prompt for Frame AI /
...
export class ModelPickerActionItem extends BaseActionViewItem {
	...
	private _getHoverContents(): IManagedHoverContent {
		...
		if (this._pickerWidget.isSetupRequired()) {
			return localize('chat.modelPicker.setupRequiredHover', "{0} • Configure a local model in the Frame sidebar — no account required.", label);
		}
