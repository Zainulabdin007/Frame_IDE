...
export class ToolsListWidget extends Disposable {
	...
	/** Read-only tool sets injected for the current session type (e.g. the local agent CLI built-ins). */
	...
	private _createHeader(): void {
		...
		DOM.append(description, $('span.section-title-description-text')).textContent = localize('toolsListSubtitle', "Enable or disable the tools available to chat. Disabled tools are not advertised to the agent. Tools other than local agent CLI run on the client and require it to be connected.");
	...
	private _createStaticReadOnlySets(): readonly IToolSet[] {
		...
		const copilotCliSet: IToolSet = {
			...
			source: ToolDataSource.Internal,
			description: localize('clientToolSet.copilotCli.description', "local agent CLI"),
			detail: localize('clientToolSet.copilotCli.detail', "Built-in tools the local agent CLI agent runs inside its own runtime."),
			getTools: () => tools,
		};
...
/**
 * The local agent CLI's built-in tools, surfaced read-only for reference. Mirrored from the published
