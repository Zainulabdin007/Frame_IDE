...
/**
 * Detects if existing hooks use local agent CLI naming convention (camelCase).
 * Returns true if any existing key matches the local agent CLI format.
 */
function usesCopilotCliNaming(hooksObj: Record<string, unknown>): boolean {
	for (const key of Object.keys(hooksObj)) {
		// Check if any key resolves to a local agent CLI hook type
			...
			switch (step) {
				case Step.SelectHookType: {
					...
					if (options?.target) {
						...
						// No target: group into Default (shared), VS Code Only, local agent CLI Only
						...
						pickerItems = [];
						if (shared.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.default', "Local/local agent CLI Agents") });
						...
						if (copilotOnly.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.copilotCliOnly', "local agent CLI Agents") });
