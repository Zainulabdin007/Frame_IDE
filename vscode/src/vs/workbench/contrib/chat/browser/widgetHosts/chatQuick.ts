...
class QuickChat extends Disposable {
	...
	private setupDisclaimer(parent: HTMLElement): void {
		...
		this._register(autorun(reader => {
			...
			if (showDisclaimer) {
				const renderedMarkdown = disposables.add(this.markdownRendererService.render(new MarkdownString(localize({ key: 'termsDisclaimer', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "Frame AI runs locally. See Frame [Terms]({2}) and [Privacy Statement]({3}) if applicable.", product.defaultChatAgent?.provider?.default?.name ?? '', product.defaultChatAgent?.provider?.default?.name ?? '', product.defaultChatAgent?.termsStatementUrl ?? '', product.defaultChatAgent?.privacyStatementUrl ?? ''), { isTrusted: true })));
