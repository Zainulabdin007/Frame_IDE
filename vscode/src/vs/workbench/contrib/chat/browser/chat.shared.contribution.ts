...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.DefaultConfiguration]: {
			...
			markdownDescription: nls.localize('chat.defaultConfiguration.settingDescription', "Controls the default configuration for new agent sessions (such as local agent sessions). You can still change the mode and approval behavior per session, and each session remembers what was used."),
		},
		[ChatConfiguration.DefaultModel]: {
			type: 'string',
			default: '',
			markdownDescription: nls.localize('chat.defaultModel.description', "The default model for new chat conversations. Use \"auto\" to let Frame AI pick a model, a model family name (such as \"opus\" or \"gemini\") to use the latest available model in that family, or a full model id. You can still switch the model within a conversation; each new conversation starts at this model."),
			policy: {
		...
		[ChatConfiguration.SessionSyncEnabled]: {
			default: false,
			markdownDescription: nls.localize('chat.sessionSync.enabled', "Enable session sync to GitHub.com. When enabled, session data is synced to your GitHub account for cross-device access and richer insights. Requires `#github.copilot.chat.localIndex.enabled#` to also be enabled."),
			type: 'boolean',
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
			...
			policy: {
				...
				localization: {
					description: {
						...
						value: nls.localize('chat.sessionSync.enabled.policy', "Enable session sync to GitHub.com for cross-device chat session history. When disabled by organization policy, session data is kept local only."),
					}
				},
			}
		},
		...
		[ClaudePreferAgentHostAgentsSettingId]: {
			type: 'boolean',
			markdownDescription: nls.localize('chat.agents.claude.preferAgentHost', "When enabled, Claude sessions opened from the Agents Window run inside the agent host process. Claude/cloud agents are not part of Frame local AI. Requires `#chat.agentHost.enabled#`."),
			default: false,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
		},
		[ClaudePreferAgentHostEditorSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.editor.claude.preferAgentHost', "When enabled, Claude sessions opened from the regular workbench run inside the agent host process. Claude/cloud agents are not part of Frame local AI."),
			default: false,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
		},
		...
		[AgentHostCustomTerminalToolEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.customTerminalTool.enabled', "When enabled, agent host SDK sessions use the Agent Host terminal tool override instead of the SDK's default terminal behavior."),
			default: false,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostCopilotSdkLogLevelSettingId]: {
			...
			enumDescriptions: [
				nls.localize('chat.agentHost.copilotSdk.logLevel.info', "Log informational messages. Running with trace logging enables all agent host SDK runtime diagnostics."),
				nls.localize('chat.agentHost.copilotSdk.logLevel.trace', "Log all agent host SDK runtime diagnostics."),
			],
			markdownDescription: nls.localize('chat.agentHost.copilotSdk.logLevel', "Controls the log level for the agent host SDK runtime. Changing this setting restarts the SDK client; active sessions are reloaded when next used."),
			default: 'info',
			scope: ConfigurationScope.APPLICATION,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostOpus48PromptEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.opus48Prompt.enabled', "When enabled, agent host SDK sessions running a Claude Opus 4.8 model apply Opus 4.8-tuned system-prompt section overrides on top of the default system message."),
			default: false,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostReasoningEffortOverrideSettingId]: {
			type: 'string',
			markdownDescription: nls.localize('chat.agentHost.reasoningEffortOverride', "Overrides the reasoning effort for agent host SDK sessions regardless of the per-model picker value. Set it to a level the selected model supports (for example `low`, `medium`, `high`, or `xhigh`) — choosing a level the model does not support may be rejected by the model. A value that isn't a recognized effort level is ignored and the session falls back to the picker value. Applied when a session is created and when its model changes. Only affects agent host CLI sessions.\n\n**Note**: This is an advanced setting for experimentation."),
			default: '',
			tags: ['experimental', 'advanced'],
		},
		[AgentHostModelCapabilityOverridesSettingId]: {
			type: 'object',
			markdownDescription: nls.localize('chat.agentHost.modelCapabilityOverrides', "Per-model capability overrides for agent host SDK sessions, keyed by model id, intended for evaluating preview models against an existing model's profile. For each model id, declare an aliased `family` (for example `claude-opus-4-8`) to route the model to that family's tuned system prompt without a code change; the model id sent to the runtime is unaffected. Only affects agent host CLI sessions.\n\n**Note**: This is an advanced setting for experimentation."),
			additionalProperties: {
				type: 'object',
				properties: {
					family: {
		...
		[AgentHostSdkSandboxEnabledSettingId]: {
			...
			markdownDescription: nls.localize('chat.agentHost.sdkSandbox.enabled', "Sandbox mode for the agent host SDK built-in shell tool. Only takes effect when `#chat.agentHost.customTerminalTool.enabled#` is `false`; when the Agent Host's own terminal tool is enabled, the engine sandbox is controlled by `#chat.agent.sandbox.enabled#`. The sandbox applies only to requests that run with default approvals — not when approvals are bypassed — and is not supported on Windows yet."),
		...
		[ChatConfiguration.BYOKUtilityModelDefault]: {
			...
			enumItemLabels: [
				...
				nls.localize('chat.byokUtilityModelDefault.copilot.label', "Frame AI"),
			],
			markdownEnumDescriptions: [
				...
				nls.localize('chat.byokUtilityModelDefault.copilot.description', "Use the default Frame AI utility models."),
			],
		...
		[PromptsConfig.HOOKS_LOCATION_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Specify paths to hook configuration files that define custom shell commands to execute at strategic points in an agent's workflow. [Learn More]({0}).\n\nRelative paths are resolved from the root folder(s) of your workspace. Supports agent hooks (`*.json`) and Claude Code hooks (`settings.json`, `settings.local.json`).",
		...
		[PromptsConfig.USE_CLAUDE_HOOKS]: {
			...
			markdownDescription: nls.localize('chat.useClaudeHooks.description', "Controls whether hooks from Claude configuration files can execute. When disabled, only Frame/agent-format hooks are used. Hooks are loaded from the files configured in `#chat.hookFilesLocations#`.",),
			default: false,
			restricted: true,
			disallowConfigurationDefault: true,
			tags: ['preview', 'prompts', 'hooks', 'agent']
		...
		[ChatConfiguration.AIDisabled]: {
			type: 'boolean',
			description: nls.localize('chat.disableAIFeatures', "Disable and hide built-in Frame AI features, including chat and inline suggestions."),
			default: false,
			scope: ConfigurationScope.WINDOW,
		},
		[ChatConfiguration.TitleBarSignInEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.titleBar.signIn.enabled', "Controls whether a title-bar sign-in button is shown. Frame AI does not require an account."),
			default: false, // Frame: local-first — never prompt to sign in
		},
		...
		'chat.extensionUnification.enabled': {
			type: 'boolean',
			description: nls.localize('chat.extensionUnification.enabled', "Legacy extension unification setting. Unused by Frame AI."),
			default: true,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		[ChatConfiguration.CollectInstructionsInExtension]: {
			type: 'boolean',
			description: nls.localize('chat.experimental.collectInstructionsInExtension', "When enabled, automatic instruction collection is performed by the chat extension instead of the core workbench."),
			default: false,
			tags: ['experimental'],
		},
