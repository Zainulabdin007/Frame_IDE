...
export interface IChatSessionsExtensionPoint {
	 ...
	 * session can own a custom pool yet still support Auto (e.g. Frame AI
	 * CLI agent host).
	 */
	...
	/**
	 * Whether this type needs a Frame AI account and so is unusable until the user signs in. Set by
	 * Copilot-backed types (local agent CLI / agent host, cloud agent) where BYOK isn't supported. Defaults to false, so
...
export interface IChatSessionsService {
	...
	/**
	 * Whether the session type needs a Frame AI account and so is unusable until the user signs in (BYOK isn't
