...
/**
 * Configuration key for enabling agent debug logging for agent host (local agent CLI) sessions.
 ...
 * agent host (local agent CLI) sessions. Registered in core (see
...
/**
 * URI scheme used by Frame AI extension for built-in skills.
 */
...
export function getSourceDescription(source: PromptFileSource): string | undefined {
	switch (source) {
		...
		case PromptFileSource.GitHubWorkspace:
			return localize('source.githubWorkspace', "Workspace (agent instructions)");
		case PromptFileSource.CopilotPersonal:
			return localize('source.copilotPersonal', "Global (agent instructions)");
