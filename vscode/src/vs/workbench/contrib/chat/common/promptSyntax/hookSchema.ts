...
/**
 * JSON Schema for Frame AI hook configuration files.
...
/**
 * Hook command schema for the local agent CLI format.
...
/**
 * Hook properties for the local agent CLI format.
 */
...
export const hookFileSchema: IJSONSchema = {
	...
	type: 'object',
	description: nls.localize('hookFile.description', 'Frame AI hook configuration file. Hooks enable executing custom shell commands at strategic points in an agent\'s workflow.'),
	additionalProperties: true,
	required: ['hooks'],
	properties: {
		hooks: {
	...
	// whether the file uses the local agent CLI format (detected by the "version" field).
	if: {
		required: ['version'],
		properties: {
			version: { type: 'number' }
	...
	then: {
		// local agent CLI format: camelCase hook names, bash/powershell/timeoutSec fields
		properties: {
			version: {
				type: 'number',
 ...
 * Only matches exact enum values. For tool-specific naming conventions (e.g., Claude, local agent CLI),
