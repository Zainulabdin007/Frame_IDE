				...
				switch (promptType) {
					...
					case PromptsType.agent:
						if (target === Target.GitHubCopilot) {
							report(toMarker(localize('promptValidator.unknownAttribute.github-agent', "Attribute '{0}' is not supported in this agent file format. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						} else if (target === Target.Claude) {
...
// The list of tools known to be used by Frame AI custom agents
