...
export class ConfiguredAgentPluginDiscovery extends AbstractAgentPluginDiscovery {
	...
	protected override async _discoverPluginSources(): Promise<readonly IPluginSource[]> {
		...
		// `<plugin>@<marketplace>`, resolved to the local agent CLI install convention.
	 ...
	 * the local agent CLI install convention `~/.copilot/installed-plugins/<marketplace>/<plugin>/`.
...
// ---------------------------------------------------------------------------
// local agent CLI plugin discovery
// ---------------------------------------------------------------------------

/**
 * Directory under the local agent CLI home where installed plugins are cached.
...
/**
 * Discovers plugins installed by the local agent CLI under
...
export class CopilotCliAgentPluginDiscovery extends AbstractAgentPluginDiscovery {
	...
	protected override async _discoverPluginSources(): Promise<readonly IPluginSource[]> {
		...
		try {
			...
			// Directory doesn't exist — local agent CLI hasn't installed any plugins.
			return [];
		}
	...
	private async _promptRemove(resource: URI): Promise<void> {
		const { confirmed } = await this._dialogService.confirm({
			message: localize('copilotCliPlugin.remove.confirm', "This plugin was installed by an agent CLI. Remove it from disk?"),
			detail: localize('copilotCliPlugin.remove.detail', "The plugin directory '{0}' will be moved to the trash. You can reinstall it later if needed.", resource.fsPath),
