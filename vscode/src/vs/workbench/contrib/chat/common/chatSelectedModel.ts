 ...
 * are served through Frame AI (CAPI) service and are therefore not BYOK.
 */
...
/**
 * Returns whether the currently selected chat model is a Copilot/CAPI model
 * (i.e. not BYOK).
 *
 * Frame: when no model is selected yet, return false so quota/upgrade chrome
 * does not treat local Frame chat as Copilot.
 */
export function isSelectedModelCopilot(
	...
	if (!vendor) {
		return false; // Frame: no selection → not Copilot
	}
