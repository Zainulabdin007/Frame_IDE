 ...
 * through setup) and the model picker (which surfaces a "Use Frame AI"
...
export function chatRequiresSetup(context: IChatSetupRequirement): boolean {
	// Frame: fully local product — never gate Chat/Agents on Microsoft/GitHub account setup.
	// Frame AI routes through frameIntelligenceService → frameOrchestratorService with no auth.
	return false;
}
 ...
 * @returns The localized full plan name (e.g., "Frame AI", "Frame AI")
 */
export function getChatPlanName(chatEntitlement: ChatEntitlement): string {
	switch (chatEntitlement) {
		case ChatEntitlement.EDU:
			return localize('plan.eduName', 'Frame AI');
		case ChatEntitlement.Pro:
			return localize('plan.proName', 'Frame AI');
		case ChatEntitlement.ProPlus:
			return localize('plan.proPlusName', 'Frame AI');
		case ChatEntitlement.Max:
			return localize('plan.maxName', 'Frame AI');
		case ChatEntitlement.Business:
			return localize('plan.businessName', 'Frame AI');
		case ChatEntitlement.Enterprise:
			return localize('plan.enterpriseName', 'Frame AI');
		default:
			return localize('plan.freeName', 'Frame AI');
	}
}
...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		// Only apply the workbench-side default if no other source (e.g. Frame AI extension)
	...
	private async onUnknownSignUpError(detail: string, logMessage: string): Promise<boolean> {
		...
		if (!this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				type: Severity.Error,
				message: localize('unknownSignUpError', "Frame AI setup failed. Local models do not require a cloud plan. Check the Frame sidebar and try again."),
				detail,
	...
	private onUnprocessableSignUpError(logMessage: string, logDetails: string): void {
		...
		if (!this.lifecycleService.willShutdown) {
			this.dialogService.prompt({
				type: Severity.Error,
				message: localize('unprocessableSignUpError', "Frame AI setup failed. Local models do not require a cloud plan."),
				detail: logDetails,
				buttons: [
					{
