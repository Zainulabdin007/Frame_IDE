...
import { FRAME_MODEL_WORKER_HOST_CHANNEL } from '../../platform/frameModelWorker/common/frameModelWorkerHost.js';
import { FrameModelWorkerHostMainService } from '../../platform/frameModelWorker/electron-main/frameModelWorkerHostMainService.js';
...
export class CodeApplication extends Disposable {
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Frame model worker host (forks tools/frame-model-worker from main — renderer is sandboxed)
		const frameModelWorkerHostChannel = ProxyChannel.fromService(
			new FrameModelWorkerHostMainService(this.logService, accessor.get(IEnvironmentMainService)),
			disposables,
		);
		mainProcessElectronServer.registerChannel(FRAME_MODEL_WORKER_HOST_CHANNEL, frameModelWorkerHostChannel);
