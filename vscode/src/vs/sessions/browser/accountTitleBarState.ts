...
function getCopilotPresentation(
	...
	if (sentiment.disabled || sentiment.untrusted) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('copilotUnavailable', "Frame AI Unavailable"),
			ariaLabel: sentiment.untrusted
				? localize('copilotUnavailableUntrustedAria', "Frame AI is unavailable in untrusted workspaces")
				: localize('copilotUnavailableDisabledAria', "Frame AI is disabled"),
		};
	}
	...
	if (entitlement === ChatEntitlement.Free && typeof remainingPercent === 'number' && remainingPercent <= 25) {
		return {
			...
			ariaLabel: localize('copilotTokensRemainingAria', "{0}% Frame AI tokens remaining", remainingPercent),
		};
	}
...
function getQuotaReachedAriaLabel(chatQuotaExceeded: boolean, completionsQuotaExceeded: boolean): string {
	if (chatQuotaExceeded && completionsQuotaExceeded) {
		return localize('copilotAllQuotaReachedAria', "Frame AI chat and inline suggestion limit reached");
	}

	if (chatQuotaExceeded) {
		return localize('copilotChatQuotaReachedAria', "Frame AI chat limit reached");
	}

	return localize('copilotCompletionsQuotaReachedAria', "Frame AI inline suggestion limit reached");
}
